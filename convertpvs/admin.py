...
class AdminAddProfile(admin.ModelAdmin):
    list_display = (
    'profile_id', 'profile_name', 'in_file_path', 'out_file_path', 'archieve_path', 'created_date', 'created_by',
    'updated_date', 'updated_by')
    list_filter = ("profile_name",)

    def __init__(self, model, admin_site):
        super().__init__(model, admin_site)
        self.opts.verbose_name_plural = "Config Profile"
...
class AdminAudit_log(ExportActionMixin, admin.ModelAdmin):
    list_display = (
    "cpv_audit_id", "user_name", "in_file_name", "out_file_name", "converted_datetime", "status", "cpv_System_Name",
    "profile_name")
    list_per_page = 20
    ...
    def __init__(self, model, admin_site):
        super().__init__(model, admin_site)
        self.opts.verbose_name_plural = "Audit Logs"
...
class CustomPeriodicTaskAdmin(PeriodicTaskAdmin):
    fieldsets = (
        (None, {'fields': ('name', 'enabled','task')}),
        ('Schedule', {'fields': ('interval', 'crontab', 'last_run_at', 'one_off')}),
    )
