    ...
    def render(self, context, menus=None):
        ...
        for group in menus:
            ...
            if len(group[key]['children']) > 0:
                r += '<li class="nav-item has-treeview"><a href="#" class="nav-link">%s <span>%s</span><span class="pull-right-container"><i class="fas fa-angle-right right"></i></span></a><ul class="treeview-menu">\n' % (
    ...
    def admin_apps(self, context, r):
        ...
        for app in context['available_apps']:
            if(str(app['app_url']) in request.path):
                r += '<li class="nav-item has-treeview menu-open"><a href="#" class="nav-link active"><i class="nav-icon fas fa-edit"></i> <p>%s </p><p><i class="fas fa-angle-right right"></i></p></a><ul class="nav nav-treeview">\n' % (app['name'])
            else:

                r += '<li class="nav-item has-treeview"><a href="#" class="nav-link"><i class="nav-icon fas fa-edit"></i> <p>%s </p><p><i class="fas fa-angle-right right"></i></p></a><ul class="nav nav-treeview">\n' % (app['name'])
                pass
...
@register.simple_tag(takes_context=False, name='menu')
